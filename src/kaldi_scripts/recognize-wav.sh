...
lat_wspecifier="ark:|lattice-scale --acoustic-scale=10.0 ark:- ark:-  >exp/lat.1"
...
run.pl JOB=1:$nj $MODEL_DIR/log/decode.JOB.log \
    nnet3-latgen-faster --frame-subsampling-factor=$frame_subsampling_factor \
     ...
     --word-symbol-table=$MODEL_DIR/words.txt "$MODEL_DIR/final.mdl" \
     $MODEL_DIR/HCLG.fst "$feats" "$lat_wspecifier" || exit 1;
